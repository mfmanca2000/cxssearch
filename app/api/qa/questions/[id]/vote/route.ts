...
export async function POST(
  ...
  try {
    ...
    const q = await query('SELECT author_dn FROM qa_questions WHERE id = $1', [id])
    if (q.rows[0]?.author_dn === session.user.dn) {
      return NextResponse.json({ error: 'Cannot vote on your own question' }, { status: 400 })
    }
